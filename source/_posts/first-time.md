...
将JsonSchema翻译为pojo的工具
其底层使用的json2java工具即为Jackson 2.x
Jsonschema2pojo定义了json schema的规范和书写方式
...
>additionalProperties为true，会生成Map作为传入参数中附加参数的接受器，false则不生成，写成“{}”等同于true。
>你也可以通过指定additionalProperties的数据类型来约束生成的结果
>schema：
...
>上面将additionalProperties的类型改为Double
...
>array写法如上即会生成Array《string》类型的参数myArrayProperty，如果在myArrayProperty指定“uniqueItems”:true，生成的集合则为Set<>.
>对于items，可以$ref引用其他jsonschema文件，也可以直接在其下编写jsonschema，会生成一个以集合参数为名称（MyArrayProperty）的对象作为该集合的泛型
...
> 这种是集合类型泛型为本schema类型，类似于tree结构
>也可以直接定位到集合的items属性作为类型
...
>如此生成会创建一个CustomTypeName类，并将属性类型指定为该类
>如果不加"type" : "object"，则不会生成对应类，但是属性类型依然为该类名，报错而已
>当然，你也可以直接指定已有的封装类
